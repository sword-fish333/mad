        ...
        <div class="card">
            <div class="card-header">
                ...
                <button type="button" class="btn btn-primary float-right" data-toggle="modal"
                        data-target="#addApartment">
                    Add apartment &nbsp; <i class="far fa-building"></i>
                </button>
                <h4 class="apartments_table_title"><u>Apartments Table</u>&nbsp;&nbsp;<i
        ...
        <table class=" data_table table table-hover  table-bordered">
            ...
            @foreach($apartments as $apartment)
                <tr>
                    ...
                    <td>
                            @if($apartment->status)
                            <strong style="color: darkred">{{$apartment->status}}</strong>
                        @php
                        $reservation=\App\Reservation::where('apartment_id', $apartment->id)->where('payment_status', 'fully')->first();

                        @endphp
                        @if($reservation)
                            Check In:{{\Carbon\Carbon::parse($reservation->check_in)->format('d-M-y h:m')}}
                            <br>
                    ...
                    <td><!-- Button trigger modal for all characteristics -->
                        <button type="button" class="btn btn-success btn-lg" data-toggle="modal"
                                data-target="#viewApartamentCharacteristics-{{$apartment->id}}">
                            <i class="fas fa-eye"></i>
                        </button>
                    </td>
                    <td><!-- Button trigger modal for edit apartment -->
                        <button type="button" class="btn btn-warning btn-lg" data-toggle="modal"
                                data-target="#editApartment-{{$apartment->id}}">
                            <i class="fas fa-edit"></i>
                        </button>
                    </td>
                    <td><a href="/admin/apartments/delete/{{$apartment->id}}" class="btn btn-danger btn-lg"
                           onclick=" return confirm('Are you sure you want to delete this Apartment?')"><i
