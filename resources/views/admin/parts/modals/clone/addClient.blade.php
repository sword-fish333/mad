    ...
    function addClient_{{$reservation->id}}(){
        ...
        var nationality=$('#nationality-{{$reservation->id}}').val();
            ...
            html+='<div class="row"><div class="form-group col-md-5">'+
                ...
                '<input type="text"  class="form-control" name="new_client_name[]"  value="'+name+'">'+
            '</div>';
        html+='<div class="form-group col-md-5">'+
            '<label class="add_reservation_info">Client Document Nr:</label>'+
            '<input type="text"  class="form-control" name="new_client_document_nr[]"  value="'+document_nr+'">'+
            '</div>';
        html+='<div class="form-group col-md-5">'+
            '<label class="add_reservation_info">Client Document Serial Nr:</label>'+
            '<input type="text"  class="form-control" name="new_client_document_serial_nr[]"  value="'+document_serial_nr+'">'+
            '</div>';

        if( $('.clone_client_document_id{{$reservation->id}}').is(':checked')){
            html += '<div class="form-group col-md-6">' +
                '<p ><strong><u>Document Type(You may choose only one)</u></strong></p>' +
                '<input type="checkbox"  class="ml-3 check_client_id-{{$reservation->id}}" name="new_document_type[]"  checked value="id_card"' +
                '>&nbsp;Id Card' +
                '<input type="checkbox"  class="ml-3 check_client_passport{{$reservation->id}}"  name="new_document_type[]" value="passport"' +
                '>&nbsp;Passport' +
                '<input type="checkbox"  class="ml-3 check_client_other{{$reservation->id}}"  name="new_document_type[]" value="other"' +
                '>&nbsp;Other' +
                '</div>';
        }else if($('.clone_client_document_passport{{$reservation->id}}').is(':checked')){
            html += '<div class="form-group col-md-6">' +
                '<p ><strong><u>Document Type(You may choose only one)</u></strong></p>' +
                '<input type="checkbox"  class="ml-3 check_client_id-{{$reservation->id}}" name="new_new_document_type[]"   value="id_card"' +
                '>&nbsp;Id Card' +
                '<input type="checkbox"  class="ml-3 check_client_passport{{$reservation->id}}"  name="new_new_document_type[]" checked value="passport"' +
                '>&nbsp;Passport' +
                '<input type="checkbox"  class="ml-3 check_client_other{{$reservation->id}}"  name="new_new_document_type[]" value="other"' +
                '>&nbsp;Other' +
                '</div>';
        }else if($('.clone_client_document_other{{$reservation->id}}').is(':checked')){
            html += '<div class="form-group col-md-6">' +
                '<p ><strong><u>Document Type(You may choose only one)</u></strong></p>' +
                '<input type="checkbox"  class="ml-3 check_client_id-{{$reservation->id}}" name="new_document_type[]"   value="id_card"' +
                '>&nbsp;Id Card' +
                '<input type="checkbox"  class="ml-3 check_client_passport{{$reservation->id}}"  name="new_document_type[]" value="passport"' +
                '>&nbsp;Passport' +
                '<input type="checkbox"  class="ml-3 check_client_other{{$reservation->id}}"  name="new_document_type[]" checked value="other"' +
                '>&nbsp;Other' +
                '</div>';
        }
        html+='<div class="form-group col-md-5">'+
            '<label class="add_reservation_info">Nationality:</label>'+
            '<input type="text"  class="form-control" name="new_nationality[]"  value="'+nationality+'">'+
            '</div>';
        html+='<div class="form-group col-md-5">'+
            '<label class="add_reservation_info"> Document Image <span class="text-danger">(Mandatory)</span>:</label>'+
            '<input type="file"  class="form-control" name="new_client_photo[]"  >'+
            '</div>';
        html+='</div>';
